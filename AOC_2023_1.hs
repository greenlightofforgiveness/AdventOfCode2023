...
import Data.List (isPrefixOf, findIndices, tails, elemIndex, elem)
...
count2 (x : xs) a       = let k = map (\q -> findString q x) digits
                              ...
                              x1 = if (index1 /= (-1)) then (findIndices (elem index1) k) !! 0 + 1 else 0
                              y1 = if (index2 /= (-1)) then (findIndices (elem index2) k) !! 0 + 1  else 0
